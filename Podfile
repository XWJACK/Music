# Uncomment this line to define a global platform for your project
platform :ios, '8.0'
# Uncomment this line if you're using Swift
use_frameworks!

target 'Music' do

#pod 'PageKit', path: '/Users/Jack/Documents/Development/Personal/Framework/Swift/PageKit'
pod 'PageKit', :git => 'git@github.com:XWJACK/PageKit.git', :commit => 'a6433da'
pod 'Wave', :git => 'git@github.com:XWJACK/Wave.git', :commit => '676b7ae'
pod 'Log', :git => 'git@github.com:XWJACK/Log.git', :commit => 'f44457e'
pod 'MusicAPI', :git => 'git@github.com:XWJACK/MusicAPI.git', :commit => '2138e41'
pod 'Lotus', :git => 'git@github.com:XWJACK/Lotus.git', :commit => '96eda6b'

pod 'SnapKit'
pod 'Alamofire'
pod 'SwiftyJSON'
pod 'Kingfisher'
pod 'Toast-Swift'
pod 'SQLite.swift'

#pod 'RealmSwift'


# OC
pod 'MJRefresh'

#Debug
pod 'Reveal-SDK', '~> 4', :configurations => ['Debug']
end
